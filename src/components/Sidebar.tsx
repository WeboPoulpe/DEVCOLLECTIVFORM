...
const links = [
  ...
  { href: '/my-answers', label: 'Réponses', icon: (
...
export function Sidebar() {
  ...
  return (
    <>
      {/* Sidebar desktop */}
      <aside className="hidden md:flex w-64 shrink-0 flex-col h-full" style={{ background: '#0f0e17' }}>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around px-1 py-2 safe-area-bottom"
        style={{ background: '#0f0e17', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
        {links.map(({ href, label, icon }) => {
          const active = pathname === href || pathname.startsWith(href + '/')
          return (
            <Link key={href} href={href}
